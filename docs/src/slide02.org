...
# Time-stamp: <2026-09-22 11:11:27 mura>
:QUARTO:
...
* R language                                                         :ignore:
:PROPERTIES:
:ID:       5F59A584-245D-4B39-AE45-523B362D1E38
:END:
#+begin_src R
  ### 第2講 資料
#+end_src
...
* 確率
** 確率分布
#+begin_callout-important :title "定義"
...
- 応用上重要な分布
   ...
   - 連続分布 (絶対連続な分布)
** 確率質量関数
#+begin_callout-note :title "離散分布を記述する方法"
...
** 確率密度関数
#+begin_callout-note :title "連続分布を記述する方法"
事象 \(A\) が起きる確率は
*確率密度関数* \(p\) の積分
\begin{equation}
  P(A)=\int_A p(x)dx
...
- 事象 \(A\) が十分小さな集合の場合
  - \(A\) に含まれる適当な点を \(x\) とし，
    ...
    \(|A|\) と書くことにすれば，
    事象 \(A\) の起きる確率を密度と事象の大きさの積で近似することができる．
    \begin{equation}
      P(A)=\int_A p(x)dx
      \simeq p(x)\cdot|A|
    \end{equation}
...
:PROPERTIES:
:ID:       D09977F8-FF8F-478B-9F64-68F07C2A08FC
:END:
:::: {.columns}

::: {.column width="45%" .fig-tall}
#+begin_src R
  #| fig-cap: "正規分布 (平均0,分散)"
  ggplot() + 
...
#+begin_src R
  #| fig-cap: "一様分布 (区間 [0,1])"
  ggplot() + 
    geom_function(fun = \(x) dunif(x,min=0,max=1),
                  colour = "red", linewidth = 2) +
    xlim(-1, 2) + ylim(0, 1.2) +
    labs(x = "x", y = "確率密度") 
#+end_src
...
::: {.column width="45%" .fig-tall}
#+begin_src R
  #| fig-cap: "Cauchy分布 (位置0,尺度1)"
  ggplot() + 
    geom_function(fun = \(x) dcauchy(x,location=0,scale=1),
                  colour = "red", linewidth = 2) +
    xlim(-6, 6) + ylim(0, 0.35) +
    labs(x = "x", y = "確率密度") 
#+end_src
:::
...
:PROPERTIES:
:ID:       E98D45AD-5EAE-4CC1-8CC8-97E2764B10DC
:END:
:::: {.columns}
...
:PROPERTIES:
:ID:       69D517E3-E045-4343-8889-ABB6CB2A4530
:END:
:::: {.columns}
...
** \(F\)分布 (\(F\)-distribution)
:PROPERTIES:
:ID:       AF6C595B-4B84-4B3B-A1FA-2615E1A0C93F
:END:
:::: {.columns}
...
#+begin_src R
  #| fig-cap: "指数分布 (比率1)"
  ggplot() + 
    geom_function(fun = \(x) dexp(x, rate = 1),
...
** 大数の強法則
#+begin_callout-important :title "定理"
\(\{X_n\}\) を確率変数列として
\begin{equation}
  S_n=\sum_{k=1}^n X_k
\end{equation}
...
** 推定量の漸近正規性
#+begin_callout-note :title "漸近正規性"
...
#+begin_callout-important :title "最尤推定量"
\(\Theta\) を尤度関数の定義域として，
...
*** 解答
:PROPERTIES:
:ID:       BA7C0016-CD4C-4CA2-8076-93E3CA22CEDD
:END:
:::: {.columns}
...
*** 解答
:PROPERTIES:
:ID:       22FFD80D-C21E-4F96-932B-E6D47C9CB105
:END:
:::: {.columns}
...
*** 解答
:PROPERTIES:
:ID:       F792FEA8-5BAF-47B0-AC1C-66AB2F5C529E
:END:
- 以下の式で計算される
   \begin{equation}
     P(\text{表の回数}\ge 15)
...
*** 解答
:PROPERTIES:
:ID:       2DB7B621-4FF0-4800-BD0A-406314CC990B
:END:
- 以下の式で計算される
   \begin{equation}
     P(\text{表の回数}\ge 15)
...
*** 解答
:PROPERTIES:
:ID:       C34295AB-5A28-4EDB-BAF3-47AA804ED4B6
:END:
- 以下の式で計算される
   \begin{equation}
     P(\text{表の回数}\ge 15)
...
*** 解答
:PROPERTIES:
:ID:       350E1889-496F-4A08-AD9D-104FEE123B7E
:END:
- 以下の式で計算される
   \begin{equation}
     P(\text{表の回数}\ge 60)
