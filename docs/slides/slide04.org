...
** 問題
   ...
   :reveal_background: #fef4f4
   :END:
   - 最小二乗推定量 $\boldsymbol{\hat{\beta}}$ について以下の問に答えなさい．
     - 平均が $\boldsymbol{\beta}$ となることを示せ．
     - 分散が $\sigma^{2}(X^{\mathsf{T}}X)^{-1}$ となることを示せ．
...
** 標準誤差
   - *標準誤差* (standard error):
     ...
     #+end_src
     #+end_quote
     - 未知パラメタ $\sigma^{2}$ は不偏分散 $\hat{\sigma}^{2}$ で推定
