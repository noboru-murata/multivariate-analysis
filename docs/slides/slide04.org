...
# Time-stamp: <2024-10-27 21:22:58 mura>
:REVEAL:
...
** 回帰係数の信頼区間
- 以下の確率変数は自由度 \(n{-}p{-}1\) の \(t\) 分布に従う
  #+begin_quote
  \begin{equation}
    \frac{\hat{\beta}_{j}-\beta_{j}}{\mathrm{s.e.}(\hat{\beta}_{j})}
    =
    \frac{(\hat{\beta}_{j}-\beta_{j})/\sqrt{\mathrm{Var}(\hat{\beta}_{j})}}{\sqrt{(n{-}p{-}1)\mathrm{s.e.}(\hat{\beta}_{j})^{2}/(n{-}p{-}1)\mathrm{Var}(\hat{\beta}_{j})}}
  \end{equation}
  #+end_quote
- \(\gamma\in(0,1)\) に対する \(\beta_{j}\) の \(1-\gamma\) 信頼区間
  #+begin_quote
  \begin{equation}
    \left[
      \hat{\beta}_{j}-t_{1{-}\gamma/2}(n{-}p{-}1)\cdot \mathrm{s.e.}(\hat{\beta}_{j}),\;
      \hat{\beta}_{j}+t_{1{-}\gamma/2}(n{-}p{-}1)\cdot \mathrm{s.e.}(\hat{\beta}_{j})
    \right]  
  \end{equation}
  #+end_quote
...
:END:
- 関連するデータの散布図
  #+begin_src R :eval no :file figs/04_pairs.png :exports results :results graphics
    #' 関連データの散布図
    tw_subset |>
      select(気温,気圧,日射,湿度,雲量) |>
      GGally::ggpairs()
...
#+reveal: split
- 観測値とあてはめ値の比較
  #+begin_src R :eval no :file figs/04_models.png :exports results :results graphics
    #' 観測値とあてはめ値の比較
    tw_subset |>
      mutate(モデル1 = fitted(tw_lm1),    # モデルごとに予測値をデータフレームに追加
             モデル2 = fitted(tw_lm2),
...
- \(t\)統計量・\(F\)統計量によるモデルの比較
  #+begin_src R :exports none
    ...
    my_gts <- function(x){
      tbl_regression(x, intercept = TRUE) |>                      # 標準の表
        ...
        add_glance_table(include = c(statistic,p.value)) }        # F統計量とp値
    tw_gt <- 
      tbl_merge(
        tbls = list(
          my_gts(tw_lm1),
  ...
  #+begin_src R :exports results :results value scalar latex :tangle no
    #' 表示(latex用)
    tw_gt |> as_gt() |>
      tab_options(table.font.size = 5) |> as_latex() |> as.character()
  #+end_src
