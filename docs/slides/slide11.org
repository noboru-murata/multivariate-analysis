...
# Time-stamp: <2026-01-06 14:15:04 mura>
:REVEAL:
...
#+begin_src R :file figs/11_hclst.png :exports results :results graphics
  ...
  par(mfrow=c(4,2), mar=rep(0.5,4), family=jp_font)
  myPlot(0)
  myPlot(1)
  myPlot(2)
  myPlot(3)
...
#+begin_src R :exports results :results value scalar latex :tangle no
  ...
  om_subset |>
    mutate(県名 = rownames(om_subset), .before = 1) |>
    gt() |>
    tab_options(table.font.size = 9, table.width = pct(80)) |>
...
- ユークリッド距離 + k-平均法 
  ...
  #+begin_src R :exports results
    ...
    for(i in 1:k){
           cat("=== cluster",i,"===\n")
           print(js_data[["都道府県名"]][js_km$cluster==i])
    }
  #+end_src
...
- ユークリッド距離 + k-メドイド法 
  ...
  #+begin_src R :exports results
    ...
    for(i in 1:k){
           cat("=== cluster",i,"===\n")
           print(js_data[["都道府県名"]][js_pam$clustering==i])
    }
  #+end_src
  ...
  my_bannerplot <- function(object, # agnesの返値
    ...
    p <- tibble(x = object[["height"]],
      ...
      scale_y_continuous(breaks = length(object[["order.lab"]]):1,
                         ...
                         labels = object[["order.lab"]],
                         position = "right") +
      labs(caption = paste("凝集係数(平均) = ",
                           round(object[["ac"]], digits = 3)))
    p
  }
  my_bannerplot(js_agnes_euc,
                fill = "red", alpha = 0.6) +
...
#+begin_src R :file figs/11_omsil7.png :exports results :results graphics :height 700
  om_data_hel |>
    ...
    silhouette() |>
    autoplot() +
    labs(x = "都道府県(クラスタ)", y = NULL,
         caption= paste("シルエット係数(平均) =",om_sil|>filter(k==7)|>pull(sil)))
#+end_src
...
#+begin_src R :file figs/11_omsil8.png :exports results :results graphics :height 700
  om_data_hel |>
    ...
    autoplot() +
    labs(x = "都道府県(クラスタ)", y = NULL,
         caption= paste("シルエット係数(平均) =",om_sil|>filter(k==8)|>pull(sil)))
#+end_src
...
#+begin_src R :file figs/11_omsil9.png :exports results :results graphics :height 700
  om_data_hel |>
    ...
    autoplot() +
    labs(x = "都道府県(クラスタ)", y = NULL,
         caption= paste("シルエット係数(平均) =",om_sil|>filter(k==9)|>pull(sil)))
#+end_src
