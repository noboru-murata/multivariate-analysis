...
# Time-stamp: <2024-11-18 11:45:52 mura>
:REVEAL:
...
:END:
- 変数間の散布図
  #+begin_src R :file figs/js_pairs_raw.png :exports results :results graphics :height 700
    #' @exercise
    #' 散布図 (標準化なし)
...
#+caption: データの散布図
#+name: fig:js_pairs_raw
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_pairs_raw.png]]
  
#+reveal: split
- 変数のばらつきに大きな違いがある
  #+begin_src R :file figs/js_boxplot_raw.png :exports results :results graphics
...
#+caption: 各変数の箱ひげ図
#+name: fig:js_boxplot_raw
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_boxplot_raw.png]]
...
:END:
- 各変数の標本平均を0，不偏分散を1に規格化する
  #+begin_src R :file figs/js_scaled_pairs.png :exports results :results graphics :height 700
    js_data |> # 散布図．いくつかの変数は相関強いことがわかる
      mutate(across(where(is.double), \(x)c(scale(x)))) |>
      GGally::ggpairs(columns = 2:6, # 都道府県名・地方区分は除く
                      legend = c(2,1), # 2行1列のグラフから凡例を作成
...
#+caption: 標準化したデータの散布図
#+name: fig:js_scaled_pairs
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_scaled_pairs.png]]
...
#+reveal: split
- 分布(密度推定)で表示する
  #+begin_src R :file figs/js_scaled_violindot.png :exports results :results graphics
    js_data |> # 箱ひげ図．変数のばらつきをそろえる
      mutate(across(where(is.double), \(x)c(scale(x)))) |>
      pivot_longer(where(is.double)) |> # 都道府県名・地方区分以外をまとめる
      ggplot(aes(x = name, y = value)) + # 既定値の name と value を利用
      geom_violindot(aes(fill = name), show.legend = FALSE) # 変数ごとに色を変える
  #+end_src

#+caption: 標準化した変数の密度推定 
#+name: fig:js_scaled_violindot
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_scaled_violindot.png]]
...
- 主成分負荷量 (標準化なし)
  ...
  #+end_src
  #+begin_src R :eval no :exports none
    js_pca_raw[["rotation"]] |> View() # 左上ペインに表として表示
  #+end_src
  #+begin_src R :exports results :results output html :tangle no
    #' データの表示(reveal用)
    js_pca_raw[["rotation"]] |>
...
- 寄与率・累積寄与率
#+begin_leftcol
#+begin_src R :file figs/js_pov_raw.png :exports results :results graphics
  js_pca_raw |>
    tidy("d") |> 
    ggplot(aes(x = PC, y = percent)) + # 各主成分(PC)ごとに寄与率(percent)を表示
    geom_bar(stat = "identity")
...
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_pov_raw.png]]
#+end_leftcol
#+begin_rightcol
#+begin_src R :file figs/js_cpov_raw.png :exports results :results graphics
  js_pca_raw |>
    tidy("d") |> 
    ggplot(aes(x = PC, y = cumulative)) + # 各主成分(PC)ごとに累積寄与率(cumlative)を表示
    geom_bar(stat = "identity")
#+end_src
#+caption: 累積寄与率 (標準化なし)
#+name: fig:js_cpov_raw
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_cpov_raw.png]]
#+end_rightcol

#+reveal: split
- 第1,2主成分得点の表示
  #+begin_src R :file figs/js_pcaplot12_raw.png :exports results :results graphics :height 700
    js_pca_raw |>
      augment(data = js_data) |> # 主成分得点のデータフレーム
      ggplot(aes(x = .fittedPC1, y = .fittedPC2, label = Pref, colour = Area)) +
      geom_point(shape = 19, size = 2) +
      ggrepel::geom_text_repel(colour = "darkgray",
                               max.overlaps = 20) +
      labs(x = "PC1", y = "PC2") 
   #+end_src

#+caption: 主成分得点による散布図 (標準化なし)
#+name: fig:js_pcaplot12_raw
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_pcaplot12_raw.png]]

#+reveal: split
- 第3,2主成分得点の表示
  #+begin_src R :file figs/js_pcaplot32_raw.png :exports results :results graphics :height 700
    js_pca_raw |>
      augment(data = js_data) |> # 主成分得点のデータフレーム
      ggplot(aes(x = .fittedPC3, y = .fittedPC2, label = Pref, colour = Area)) +
      geom_point(shape = 19, size = 2) +
      ggrepel::geom_text_repel(colour = "darkgray",
                               max.overlaps = 20) +
      labs(x = "PC3", y = "PC2") 
...
#+caption: 主成分得点による散布図 (標準化なし)
#+name: fig:js_pcaplot32_raw
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_pcaplot32_raw.png]]

#+reveal: split
- 主成分負荷量 (標準化あり)
  #+begin_src R :eval no :exports none
    js_pca[["rotation"]] |> View() # 左上ペインに表として表示
  #+end_src
  #+begin_src R :exports results :results output html :tangle no
    #' データの表示(reveal用)
    js_pca[["rotation"]] |>
...
- 寄与率・累積寄与率
#+begin_leftcol
#+begin_src R :file figs/js_pov.png :exports results :results graphics
  js_pca |>
    tidy("d") |> 
    ggplot(aes(x = PC, y = percent)) + # 各主成分(PC)ごとに寄与率(percent)を表示
    geom_bar(stat = "identity")
...
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_pov.png]]
#+end_leftcol
#+begin_rightcol
#+begin_src R :file figs/js_cpov.png :exports results :results graphics
  js_pca |>
    tidy("d") |> 
    ggplot(aes(x = PC, y = cumulative)) + # 各主成分(PC)ごとに累積寄与率(cumlative)を表示
    geom_bar(stat = "identity")
...
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_cpov.png]]
#+end_rightcol

#+reveal: split
- 第1,2主成分得点の表示
  #+begin_src R :file figs/js_pcaplot12.png :exports results :results graphics :height 700
    js_pca |>
      augment(data = js_data) |> # 主成分得点のデータフレーム
      ggplot(aes(x = .fittedPC1, y = .fittedPC2, label = Pref, colour = Area)) +
      geom_point(shape = 19, size = 2) +
...
#+caption: 主成分得点による散布図 (標準化なし)
#+name: fig:js_pcaplot12
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_pcaplot12.png]]

#+reveal: split
- 第3,2主成分得点の表示
  #+begin_src R :file figs/js_pcaplot32.png :exports results :results graphics :height 700
    js_pca |>
      augment(data = js_data) |> # 主成分得点のデータフレーム
      ggplot(aes(x = .fittedPC3, y = .fittedPC2, label = Pref, colour = Area)) +
      geom_point(shape = 19, size = 2) +
...
- 寄与率
  #+begin_src R :eval no :exports none
    summary(js_pca)[["importance"]] |> View() # 左上ペインに表として表示
  #+end_src
  #+begin_src R :exports results :results output html :tangle no
    #' データの表示(reveal用)
    summary(js_pca)[["importance"]] |>
      as.data.frame() |> rownames_to_column() |>
      gt() |>
      fmt_number(decimals = 4) |>
      as_raw_html()
...
#+reveal: split
- 第1,2主成分によるバイプロット
  #+begin_src R :file figs/js_biplot12.png :exports results :results graphics :height 700
...
#+name: fig:js_biplot12
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_biplot12.png]]

#+reveal: split
- 第3,2主成分によるバイプロット
  #+begin_src R :file figs/js_biplot32.png :exports results :results graphics :height 700
    autoplot(js_pca, 
             x = 3, y = 2, # x軸を第3，y軸を第2
             data = js_data,                  # 地方区分などの補助情報を渡す
             colour = "Area",                 # 地方区分ごとに色付けする
...
#+reveal: split
- 中心部の拡大 (第1,2主成分)
  #+begin_src R :file figs/js_biplot12e.png :exports results :results graphics :height 700
    autoplot(js_pca, 
             data = js_data,                  # 地方区分などの補助情報を渡す
             colour = "Area",                 # 地方区分ごとに色付けする
             shape = 19,                      # 以下データ点の修飾．点の形
...
#+caption: 主成分分析のバイプロット(第1,2)
#+name: fig:js_biplot12e
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_biplot12e.png]]

#+reveal: split
- 中心部の拡大 (第3,2主成分)
  #+begin_src R :file figs/js_biplot32e.png :exports results :results graphics :height 700
    autoplot(js_pca, 
             x = 3, y = 2, # x軸を第3，y軸を第2
             data = js_data,                  # 地方区分などの補助情報を渡す
             colour = "Area",                 # 地方区分ごとに色付けする
...
#+caption: 主成分分析のバイプロット(第3,2)
#+name: fig:js_biplot32e
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_biplot32e.png]]
