...
# Time-stamp: <2023-11-21 12:19:24 mura>
:REVEAL:
...
- スペクトル分解との関係
  ...
  \begin{align}
    \text{(全体の変動)}
    &=\sum_{i=1}^{n}\|\boldsymbol{x}_i-\bar{\boldsymbol{x}}\|^2 \\
    &=\sum_{i=1}^{n}(\boldsymbol{x}_i-\bar{\boldsymbol{x}})^{\mathsf{T}}
      (\boldsymbol{x}_i-\bar{\boldsymbol{x}})\\
    &=\mathrm{tr}XX^{\mathsf{T}}=\mathrm{tr}X^{\mathsf{T}}X\\
    &=\mathrm{tr}\sum_{k=1}^{p}\lambda_{k}
      \boldsymbol{a}_{k}\boldsymbol{a}_{l}^{\mathsf{T}}
      =\sum_{k=1}^{p}\lambda_{k}\mathrm{tr}
      \boldsymbol{a}_{k}\boldsymbol{a}_{l}^{\mathsf{T}}
      =\sum_{k=1}^{p}\lambda_{k}\boldsymbol{a}_{l}^{\mathsf{T}}\boldsymbol{a}_{k}\\
    &=\sum_{k=1}^p\lambda_{k}
  \end{align}
  #+end_quote
...
- 第1,2主成分得点の表示
  #+begin_src R :file figs/07_pcaplot12_raw.png :exports results :results graphics
    autoplot(js_pca_raw, data = js_data, colour = "Area", # 地方区分ごとに色を変える
             scale = 0, asp =1,
             shape = 19, size = 2, 
             loadings = FALSE) +
      ggrepel::geom_text_repel(aes(label = Pref), colour = "darkgray",
