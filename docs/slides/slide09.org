...
#+DATE: 
# Time-stamp: <2022-11-21 17:48:19 mura>
:REVEAL:
...
#+reveal: split
- テスト
  #+begin_src R :exports results :results html :tangle yes
    library(xtable)
    print(xtable(confusionMatrix(tw_qest$class, factor(tw_subset$month))$table),type="html")
  #+end_src
...
- 実際のデータの一部 (続き) 
  #+begin_src R :exports results :tangle yes
    head(wq_data[,9:ncol(wq_data)], 16)
  #+end_src
...
- 予測誤差の比較 (線形/2次)
  #+begin_src R :exports results :tangle yes
    confusionMatrix(wq_ldloo$class, wq_data$quality)$overall[1:2]
    confusionMatrix(wq_ldloo$class, wq_data$quality)$byClass[,c(1,2,5,6,7)]
  #+end_src
  #+begin_src R :exports results :tangle yes
    confusionMatrix(wq_qdloo$class, wq_data$quality)$overall[1:2]
    confusionMatrix(wq_qdloo$class, wq_data$quality)$byClass[,c(1,2,5,6,7)]
  #+end_src
  - 予測誤差の観点からは線形判別の方が良さそう
