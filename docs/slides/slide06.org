...
# Time-stamp: <2025-11-17 17:16:39 mura>
:REVEAL:
...
#+begin_src R :exports results :results value scalar latex :tangle no
  ...
  ja_data |>
    gt() |>
    tab_caption("都道府県別の人口関連データ") |>
    tab_options(table.font.size = 9, table.width = pct(90)) |>
    as_latex() |> as.character()
#+end_src
...
#+begin_src R :file figs/js_boxplot.png :exports results :results graphics
  js_data |> # 箱ひげ図．
    ...
    ggplot(aes(x = name, y = value)) + # 既定値の name と value を利用
    geom_boxplot(aes(fill = name), alpha = 0.5,
                 show.legend = FALSE) +# 変数ごとに色を変える
    labs(x = NULL, y = NULL)
#+end_src
